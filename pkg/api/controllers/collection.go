...
func (controller *Collection) Create(ctx *handler.Context) {
	...
	createModel := collection.NewCreateModelFromRequest(ctx.Request)
...
func (controller *Collection) Update(ctx *handler.Context) {
	...
	updateModel := collection.NewUpdateModelFromRequest(ctx.Request)

	updateModel.ID = uint(id)
	updateModel.UserID = user.ID
